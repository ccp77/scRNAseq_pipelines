DimRed <- function( outputSuffix = "DimRed/out_minD0.05",
       DotSNE = T,
       ...
       kValue = 30, 
       names2_file = "Srt_Shah/Srt_shah_names2.csv",
       embedding_file_folder = "Srt_Shah/Srt_shah_seurat_pca_for_DimRed.csv") {
  ...
  ##### Main Parameters --------------------------------------------------------------------------
  sourceFcsFolder <- embedding_file_folder     # Folder with samples (even 1 sample ok)
  ceil = 50000000                  # Number of events to take PER SAMPLE (unless sample has less)
  FNnames <- names2_file
  OutputSuffix <- outputSuffix
  #FNnames <- paste("../", names2_file, sep='') # Parameters to analyze in csv file
  #OutputSuffix <- paste("../", outputSuffix, sep='')        # Adds this to all new directories and file names
  ...
  DoUMAP <-  DoUMAP
  min_dist <-  min_dist_UMAP          #default = 0.2
  n_neighbors <-  n_neighbors_UMAP         #default = 15
