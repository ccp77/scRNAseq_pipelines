...
FCStSNEone <- function (LoaderPATH ="fcs", #Folder with samples (even 1 sample ok)
  ...
  if(!useCSV){
    for (FFs in 1:NumBC){
      ...
      #suppressWarnings(dir.create(paste0(LoaderPATH,"_",OutputSuffix))) ### CECILE MODIFIED PATH HERE
      suppressWarnings(dir.create(paste0(OutputSuffix,"_csv")))
    ...
    {
      #suppressWarnings(dir.create(paste0(LoaderPATH,"_",OutputSuffix,"_csv"))) ### CECILE MODIFIED PATH HERE
      suppressWarnings(dir.create(paste0(OutputSuffix,"_csv")))
      for (FFs in 1:NumBC){
        ...
        BaseFN <- sapply(strsplit(FcsFileNames[FFs], split ="\\."), "[", 1)
        #FNresult <- paste0(LoaderPATH,"_",OutputSuffix,"/",BaseFN,"_",OutputSuffix,".fcs") ### CECILE MODIFIED EXPORT PATH
        FNresult <- paste0(OutputSuffix,"/",OutputSuffix,".fcs")
        #FNresultCsv <- paste0(LoaderPATH,"_",OutputSuffix,"_csv","/",BaseFN,"_",OutputSuffix,".csv") ### CECILE MODIFIED EXPORT PATH
        OutputSuffix_notPath <- gsub("/","_",OutputSuffix)
        FNresultCsv <- paste0(OutputSuffix,"_csv","/",OutputSuffix_notPath,".csv")
